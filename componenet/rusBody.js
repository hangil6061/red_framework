...
Red.RusBody = (function ()
{
    ...
    Body.prototype.eventUpdate = function ()
    {
        if( this.isCallEvent )
        {
            var i = 0;
            this.tempColArr.length = 0;
            for( i = 0; i < this.colArr.length; i++ )
            {
                var idx = this.preColArr.indexOf( this.colArr[i] );
                if( idx > -1 )
                {
                    //스테이.
                    this.preColArr.splice( idx, 1 );
                    if( this.stayEventCall.length > 0 )
                    {
                ...
                {
                    //엔터
                    for(var j = 0; j < this.enterEventCall.length; j++)
                    {
                        this.enterEventCall[j](this.colArr[i]);
                    }
                }
            }

            for( i = 0; i < this.preColArr.length; i++ )
            {
               //엑시트
                for(var j = 0; j < this.exitEventCall.length; j++)
                {
                    this.exitEventCall[j](this.preColArr[i]);
                }
            }

            this.preColArr.length = 0;
            var temp = this.preColArr;
            this.preColArr = this.colArr;
            this.colArr = temp;
            temp = null;
        }
    },
